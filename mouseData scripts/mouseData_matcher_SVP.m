...
for a = 1:(num_sessions_to_align)
    ...
    for b = 1:(map_height)
        if (exists_map(b) == 1)
            ...
            % disp(index)
            % disp(exists_map(b))
            matchedMouseData(a).ca.allNormTraces(start_index) = ogMouseData(a).ca.allNormTraces(index);
            matchedMouseData(a).ca.caData(start_index) = ogMouseData(a).ca.caData(index);
...
clear a b matchedMouseData caDataLength exists i index j k exists_map map map_height normTracesLength num_cells num_sessions_to_align ogMouseData start_index
